"""Practical-significance tiering for Pass-2 Wilcoxon results."""
...
def load_iaa_alpha(iaa_path: Path | None) -> dict[str, float]:
    if iaa_path is None or not iaa_path.exists():
        return {}
    iaa = pd.read_csv(iaa_path)
...
def classify_tier(row: pd.Series, iaa_map: dict[str, float]) -> str:
    ...
    if sig and mean >= PRIMARY_MEAN and pct >= PRIMARY_PCT and has_validation:
        return "primary"
    if sig:
        return "exploratory"
