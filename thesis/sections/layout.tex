...
It is very important that the references in a document conform to a particular standard, because it must be possible for readers to find the background material referred to. The author, year, title of the book/journal, publisher and issue number must all be provided. The KU template defaults to APA author-year conventions; this project uses \textbf{numeric superscript} citations instead, per supervisor approval (see \texttt{thesis/CITATION.md}).

In the compiled thesis, in-text markers are superscript numbers (e.g.\ after a claim \cite{schwartz97}), with full bibliographic entries in the numbered reference list. Use at most one citation or cross-reference anchor per sentence.
